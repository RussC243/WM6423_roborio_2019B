...
public class Pneumatics {
  ...
  public Pneumatics(OurBots selectedBot)// constructor
  {
    ...
    case WM2019_BAG:
    default:
      // Compressor code is handled internally. Make Solenoid objects and features
      // from there.
      ...
      // There are 3 pnuematic control devices, each with 2 solenoids. One extends
      // piston and one retracts piston.
      // zzz pneumatic_front_up = new Solenoid(hMap.canID_PCM,
      // hMap.pnuematic_front_up); //CAN ID for PCM, position on PCM
      // zzz pneumatic_front_down = new Solenoid(hMap.canID_PCM,
      // hMap.pnuematic_front_down);
      // zzz pneumatic_rear_up = new Solenoid(hMap.canID_PCM, hMap.pnuematic_rear_up);
      // zzz pneumatic_rear_down = new Solenoid(hMap.canID_PCM,
      // hMap.pnuematic_rear_down);
      ...
      // zzz pneumatic_front_up.setPulseDuration(PULSE_DURATION); //PCM emits a pulse
      // lasting 1.01 seconds.
  ...
  public void hatchRetract() {
    ...
    System.out.println("Hatch retracted");
  }
  ...
  public void regulate_pnuematic_MotorsCurrent() { /**
                                                    * Method for monitoring current of pneumatic motors, once it exceeds
                                                    * a certain value. We might need to stall it.
                                                    */
    if (panel.getCurrent(hMap.PDP_climbWheelLeft) > MOTOR_CURRENT_LIMIT) {
      ...
      try {
        pneumaticWheelLeft.set(0);
        pneumaticWheelLeft.wait(1000);
    ...
    if(panel.getCurrent(hMap.PDP_climbWheelRight) > MOTOR_CURRENT_LIMIT )
    {
      System.out.println("Pneumatic motor's current has exceeded 3A");
      try {
        pneumaticWheelRight.set(0);
        pneumaticWheelRight.wait(1000);
      } catch (InterruptedException e) {
        e.printStackTrace();
      }
      //Do something.  
